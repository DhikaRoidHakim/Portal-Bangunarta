...
class _TransaksiScreenState extends ConsumerState<TransaksiScreen> {
  ...
  void _onScroll() {
    if (_scrollController.position.pixels >=
      ...
      ref.read(sambaTransaksiNotifierProvider.notifier).loadMore();
    }
  }
  ...
  void _onSearchChanged(String query) {
    ...
    _debounce = Timer(const Duration(milliseconds: 500), () {
      if (mounted) {
        ref.read(sambaTransaksiSearchQueryProvider.notifier).updateQuery(query);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return RefreshIndicator(
      ...
      onRefresh: () async {
        ref.invalidate(sambaTransaksiNotifierProvider);
      },
                        ...
                        return Padding(
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const Text(
                                'Tidak ditemukan data transaksi yang cocok atau terdaftar.',
                              ...
                              ElevatedButton(
                                onPressed: () {
                                  ref.invalidate(
                                    sambaTransaksiNotifierProvider,
                                  );
                                },
  ...
  Widget _buildTransactionItem(
    ...
    SambaTransactionModel item, {
    bool isLast = false,
  }) {
    final isPending = item.status == 'Belum Diotorisasi';
    final bgColor = isPending
        ? const Color(0xFFFEF7E9) // Kuning lembut
        : const Color(0xFFE8F5E9); // Hijau lembut
    final statusColor = isPending
        ? const Color(0xFFD97706) // Text amber/kuning gelap
        : const Color(0xFF15803D); // Text hijau gelap
    final statusBg = isPending
        ? const Color(0xFFFEF3C7) // Badge amber/kuning
        : const Color(0xFFDCFCE7); // Badge hijau

    return InkWell(
      onTap: () {
        context.push('/samba/transaksi', extra: item.id);
      },
      child: Container(
        color: bgColor,
